...
class InitTool{
  ...
  getUserLocation() async {
    await ATInitManger.getUserLocation().then((value) {
      jcRizhi('flutter: Get user location -- ${value.toString()}');
    });
  }
  // 获取GDPR的授权级别
  getGDPRLevel() async {
    await ATInitManger.getGDPRLevel().then((value) {
      jcRizhi('flutter:Get GDPR --${value.toString()}');
    });
  }
      ...
      if (value.userLocation != null) {
        switch (value.userLocation) {
          case InitUserLocation.initUserLocationInEU:
            jcRizhi("flutter Monitor initial user location in the EU--");
          ...
          case InitUserLocation.initUserLocationOutOfEU:
            jcRizhi("flutter: flutter The location of the listening initial user is not in the EU");
            break;
          case InitUserLocation.initUserLocationUnknown:
            jcRizhi("flutter: flutter The location of the initial listening user is unknown");
            break;
        }
      }

      if (value.consentDismiss != null) {
        jcRizhi("flutter: flutter consent dismiss callback");
      }
  ...
  loadInterstitialAd({
    ...
    await ATInterstitialManager
        .loadInterstitialAd(placementID: placementId, extraMap: {});
  }
  ...
  hasInterAdReady({
    ...
    bool result =  await ATInterstitialManager
        .hasInterstitialAdReady(
      placementID: placementId,
    );
    jcRizhi('===topon===flutter插屏广告视频缓存hasInterAdReady:$result');
    return result;
  }
  ...
  //
  // 有关插屏广告事件的通知示例如下:
  interstitialListener(void Function(ATInterstitialResponse)? atInterstitialResponse) {
  ...
  loadRewardedVideo({
    ...
    jcRizhi("====topon激励=====loadRewardedVideo");
  ...
  hasRewardAdReady({
    ...
    bool result =  await ATRewardedManager
        .rewardedVideoReady(
      placementID: placementId,
    );
    jcRizhi('==topon===广告视频缓存hasRewardAdReady:$result');
    return result;
  }
